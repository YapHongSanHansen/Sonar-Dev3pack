...
async function analyzeHandler(request: FastifyRequest, reply: FastifyReply) {
  let payload;
  try {
...
export async function riskRoute(fastify: FastifyInstance) {
  ...
  // Legacy alias — kept so existing clients on /risk don't break. Prefer /analyze in new code.
  fastify.post('/risk', analyzeHandler);
}
